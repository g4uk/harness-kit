...
if [ "$UPDATE_MODE" = false ]; then
  ...
  echo "   3. CI: add CLAUDE_CODE_OAUTH_TOKEN (preferred — subscription quota, run"
  echo "      'claude setup-token' locally to generate it) or ANTHROPIC_API_KEY (metered,"
  echo "      fallback) as a REPOSITORY secret (Settings > Secrets and variables > Actions"
  echo "      > Repository secrets) for harness-evals + agent-review. NOT an Environment"
  echo "      secret — these workflows don't declare environment:, so an environment-scoped"
  echo "      (or org-scoped-but-unshared) secret silently yields apiKeySource:none in the"
  echo "      container, not an error."
